    ...
    if created:
        if instance.total_returned_qty > 0:
            ...
            if credit_note.exists():
                ...
                credit_note.amount= credit_note.amount + (instance.total_returned_qty * instance.returned_product.product_pro_price.get(shop__shop_type__shop_type='sp', status=True).price_to_retailer)
                credit_note.save()
            else:
                credit_note = Note.objects.create(credit_note_id = brand_credit_note_pattern(instance.return_id.pk), order=instance.return_id.invoice_no.order,return_no = instance.return_id, amount = instance.total_returned_qty * instance.returned_product.product_pro_price.get(shop__shop_type__shop_type='sp', status=True).price_to_retailer, status=True)
