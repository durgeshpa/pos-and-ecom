            ...
            if form.is_valid():
                ...
                if trip_status == Trip.STARTED:
                    if current_trip_status == Trip.COMPLETED:
                        ...
                        OrderedProductMapping.objects.filter(ordered_product__in=trip_instance.rt_invoice_trip.filter(shipment_status=TRIP_SHIPMENT_STATUS_MAP[current_trip_status])).update(delivered_qty=(F('shipped_qty') - (F('damaged_qty') + F('returned_qty'))))
                    else:
