...
class ProductOptionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = ProductOption
        fields = ('size', 'package_size', 'color',
                  'fragrance', 'flavor', 'weight')
...
class ProductsSearchSerializer(serializers.ModelSerializer):
    ...
    product_case_size_picies = serializers.SerializerMethodField(
        'product_case_size_picies_dt')
...
class CartDataSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Cart
        fields = ('id', 'order_id', 'cart_status',
...
class CartProductMappingSerializer(serializers.ModelSerializer):
    ...
    product_sub_total = serializers.SerializerMethodField(
        'product_sub_total_dt')

    def is_available_dt(self, obj):
        ordered_product_sum = OrderedProductMapping.objects.filter(
            product=obj.cart_product).aggregate(available_qty_sum=Sum('available_qty'))
    ...
    def product_sub_total_dt(self, obj):
        shop_id = self.context.get("parent_mapping_id", None)
        product_price = 0 if obj.cart_product.product_pro_price.filter(shop__id=shop_id).last() is None else round(
            obj.cart_product.product_pro_price.filter(shop__id=shop_id).last().price_to_retailer, 2)
    ...
    class Meta:
        ...
        fields = ('id', 'cart', 'cart_product', 'qty', 'qty_error_msg',
                  'is_available', 'no_of_pieces', 'product_sub_total')
...
class CartSerializer(serializers.ModelSerializer):
    ...
    def total_amount_id(self, obj):
        ...
        for cart_pro in obj.rt_cart_list.all():
            ...
            shop_id = self.context.get("parent_mapping_id", None)
            pro_price = ProductPrice.objects.filter(
                shop__id=shop_id, product=cart_pro.cart_product).last()
            if pro_price and pro_price.price_to_retailer:
                self.total_amount = float(self.total_amount) + (float(pro_price.price_to_retailer) * float(
                    cart_pro.qty) * float(pro_price.product.product_inner_case_size))
            else:
...
class NoteSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Note
        fields = ('id', 'amount', 'note_type', 'last_modified_by',
                  'created_at', 'modified_at', 'note_link')
...
class CustomerCareSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = CustomerCare
        fields = ('name', 'email_us', 'contact_us', 'order_id',
                  'order_status', 'select_issue', 'complaint_detail')
...
class GramMappedCartProductMappingSerializer(serializers.ModelSerializer):
    ...
    def is_available_dt(self, obj):
        ordered_product_sum = GRNOrderProductMapping.objects.filter(
            product=obj.cart_product).aggregate(available_qty_sum=Sum('available_qty'))
        self.is_available = True if ordered_product_sum['available_qty_sum'] and int(
            ordered_product_sum['available_qty_sum']) > 0 else False
        return self.is_available
    ...
    def product_sub_total_dt(self, obj):
        shop_id = self.context.get("parent_mapping_id", None)
        product_price = 0 if obj.cart_product.product_pro_price.filter(shop__id=shop_id).last(
        ) is None else obj.cart_product.product_pro_price.filter(shop__id=shop_id).last().price_to_retailer
        return float(obj.cart_product.product_inner_case_size) * float(obj.qty) * float(product_price)
...
class GramMappedCartSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        representation = super(GramMappedCartSerializer,
...
class GramMappedOrderedProductSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('order', 'invoice_no', 'vehicle_no', 'shipped_by', 'received_by',
                  'last_modified_by', 'created_at', 'modified_at', 'rtg_order_product_note', 'invoice_link')
...
class GramMappedOrderSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        representation = super(GramMappedOrderSerializer,
                               self).to_representation(instance)
        representation['created_at'] = instance.created_at.strftime(
            "%Y-%m-%d - %H:%M:%S")
        return representation
