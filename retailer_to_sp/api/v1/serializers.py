...
from retailer_to_sp.models import (CartProductMapping, Cart, Invoice, LastMileTripReturnMapping, Order, OrderedProduct, Note, CustomerCare, Payment,
                                   ...
                                   RETURN_REMARK_CHOICES, add_to_putaway_on_partail, ReturnOrder, CartOffers)
...
from shops.api.v1.serializers import SellerShopSerializer
...
class OrderedProductMappingSerializer(serializers.ModelSerializer):
    ...
    def get_product_price(self, obj):
        ...
        cart_product_mapping = CartProductMapping.objects.filter(cart_product=obj.product,
...
class ProductsSearchSerializer(serializers.ModelSerializer):
    ...
    def product_price_dt(self, obj):
        current_price = obj.get_current_shop_price(self.context.get('parent_mapping_id'),
                                                   self.context.get('buyer_shop_id'))
        if current_price:
    ...
    def margin_dt(self, obj):
        if self.product_mrp:
            return round((((float(self.product_mrp) - float(self.product_price) / int(
                obj.product_inner_case_size)) / float(self.product_mrp)) * 100), 2)
        return False
...
class CartProductMappingSerializer(serializers.ModelSerializer):
    ...
    def is_available_dt(self, obj):
        ...
        """
        ordered_product_sum = OrderedProductMapping.objects.filter(product=obj.cart_product).aggregate(
            available_qty_sum=Sum('available_qty'))
        self.is_available = True if ordered_product_sum['available_qty_sum'] and int(
            ordered_product_sum['available_qty_sum']) > 0 else False
    ...
    def product_sub_total_dt(self, obj):
        product_price = obj.cart_product.get_current_shop_price(self.context.get('parent_mapping_id'),
                                                                self.context.get('buyer_shop_id'))
    ...
    def product_coupons_dt(self, obj):
        ...
        for rules in obj.cart_product.purchased_product_coupon.filter(rule__is_active=True,
                                                                      rule__expiry_date__gte=date). \
                exclude(rule__coupon_ruleset__shop__shop_type__shop_type='f'):
            for rule in rules.rule.coupon_ruleset.filter(is_active=True, expiry_date__gte=date).exclude(
        ...
        if product_coupons:
            ...
            coupons_queryset2 = Coupon.objects.filter(coupon_code__in=product_coupons, coupon_type='brand').order_by(
                'rule__cart_qualifying_min_sku_value')
    ...
    def total_amount_id(self, obj):
        ...
        for cart_pro in obj.rt_cart_list.all():
            ...
            pro_price = cart_pro.cart_product.get_current_shop_price(self.context.get('parent_mapping_id'),
                                                                     self.context.get('buyer_shop_id'))
            if pro_price:
                self.total_amount += Decimal(pro_price.get_per_piece_price(cart_pro.qty)) * Decimal(
                    cart_pro.no_of_pieces)
...
class SuperStoreCartSerializer(serializers.ModelSerializer):
    ...
    def get_total_quantity(self, instance):
        return instance.rt_cart_list.filter(product_type=1).aggregate(total_quantity=Sum('qty')).get('total_quantity',
                                                                                                     0)
...
class OrderSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Order
        fields = (
            'id', 'ordered_cart', 'order_no', 'billing_address', 'shipping_address', 'total_mrp',
...
class ProductsSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Product
        fields = (
            'id', 'product_name', 'product_slug', 'product_short_description', 'product_long_description',
            'product_sku',
...
class OrderedCartSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Cart
        fields = (
            'id', 'order_id', 'cart_status', 'created_at', 'modified_at', 'rt_cart_list', 'total_amount', 'sub_total',
            'items_count', 'offers', 'total_discount')
...
class ProductsSearchListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Product
        fields = (
            'id', 'product_name', 'product_sku', 'product_inner_case_size', 'product_case_size',
            'product_case_size_picies')
...
class OrderedCartProductMappingListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = CartProductMapping
        fields = (
            'id', 'cart', 'cart_product', 'qty', 'qty_error_msg', 'no_of_pieces', 'product_sub_total',
            'cart_product_price',
            'product_inner_case_size')
...
class OrderListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Order
        fields = (
            'id', 'ordered_cart', 'order_no', 'total_final_amount', 'order_status', 'shipment_status',
            'shipping_address',
            'created_at', 'modified_at', 'rt_order_order_product')
...
class CustomerCareSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = CustomerCare
        fields = (
            'phone_number', 'complaint_id', 'email_us', 'order_id', 'issue_status', 'select_issue', 'complaint_detail')
...
class GramMappedCartProductMappingSerializer(serializers.ModelSerializer):
    ...
    def is_available_dt(self, obj):
        ordered_product_sum = GRNOrderProductMapping.objects.filter(product=obj.cart_product).aggregate(
            available_qty_sum=Sum('available_qty'))
        self.is_available = True if ordered_product_sum['available_qty_sum'] and int(
            ordered_product_sum['available_qty_sum']) > 0 else False
        return self.is_available
    ...
    def product_sub_total_dt(self, obj):
        ...
        product_price = 0 if obj.cart_product.product_pro_price.filter(
            shop__id=shop_id).last() is None else obj.cart_product.product_pro_price.filter(
            shop__id=shop_id).last().price_to_retailer
    ...
    class Meta:
        ...
        fields = (
            'id', 'cart', 'cart_product', 'qty', 'qty_error_msg', 'is_available', 'no_of_pieces', 'product_sub_total')
        ...
        for cart_pro in obj.rt_cart_list.all():
            ...
            if pro_price and pro_price.price_to_retailer:
                self.total_amount = float(self.total_amount) + (
                        float(pro_price.price_to_retailer) * float(cart_pro.qty) * float(
                    pro_price.product.product_inner_case_size))
            else:
...
class GramMappedOrderedProductSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('order', 'invoice_no', 'vehicle_no', 'shipped_by', 'received_by', 'last_modified_by', 'created_at',
                  'modified_at', 'rtg_order_product_note', 'invoice_link')
...
class GramMappedOrderSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = (
            'id', 'ordered_cart', 'order_no', 'billing_address', 'shipping_address', 'total_mrp',
            'total_discount_amount',
            'total_tax_amount', 'total_final_amount', 'order_status', 'shipment_status', 'ordered_by', 'received_by',
            'last_modified_by',
            'created_at', 'modified_at', 'rt_order_order_product')
...
class ShipmentSerializer(serializers.ModelSerializer):
    ...
    def get_shop_id(self,obj):
        return obj.order.buyer_shop.id
    ...
    class Meta:
        model = OrderedProduct
        fields = ('item_type','shop_id','shipment_id', 'invoice_no', 'shipment_status', 'payment_mode', 'invoice_amount', 'order',
...
class ShipmentDetailSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = RetailerOrderedProductMapping
        fields = (
            'ordered_product', 'ordered_product_status', 'product', 'product_short_description', 'mrp',
            'price_to_retailer',
...
class TripSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Trip
        fields = (
            'trip_id', 'dispatch_no', 'trip_status', 'no_of_shipments', 'total_trip_amount', 'cash_to_be_collected',
            'trip_return_amount')
...
class CustomerShipmentReturnOrderTripDetailSerializer(serializers.ModelSerializer):
    ...
    def get_ordered_qty(self, instance):
        return instance.shipped_qty

    class Meta:
        model = RetailerOrderedProductMapping
        fields = ('id', 'shipped_qty', 'delivered_qty', 'ordered_qty')
    ...
    def get_customer_shipment_detail(self, instance):
        return CustomerShipmentReturnOrderTripDetailSerializer(
            instance.ref_return_order.shipment \
                .order.rt_order_order_product.last() \
            ...
            if 'status' in self.initial_data and self.initial_data['status']:
                ...
                if status == shipment_status:
                    ...
                    if status == OrderedProduct.NOT_ATTEMPT:
                        ...
                        or (
                        shipment_status == OrderedProduct.READY_TO_SHIP and status != OrderedProduct.MOVED_TO_DISPATCH):
                    raise serializers.ValidationError(f'Invalid status | {shipment_status}-->{status} not allowed')
                ...
                if status in [OrderedProduct.QC_STARTED, OrderedProduct.READY_TO_SHIP] and \
                    ...
                    product_qc_pending = shipment.rt_order_product_order_product_mapping.filter(
                        is_qc_done=False).first()
                    raise serializers.ValidationError(f'QC is not yet completed for {product_qc_pending.product}')
                        ...
                        shipment.shipment_packaging.filter(
    ...
    def create_shipment_reschedule(self, shipment_instance, rescheduling_reason, rescheduling_date, trip):
        ...
        if not ShipmentRescheduling.objects.filter(shipment=shipment_instance).exists():
            ShipmentRescheduling.objects.create(
                shipment=shipment_instance, rescheduling_reason=rescheduling_reason,
...
class ShipmentCratesSerializer(serializers.ModelSerializer):
    status = serializers.SerializerMethodField()
    crate = serializers.SerializerMethodField()
    is_return_verified = serializers.SerializerMethodField()
    id = serializers.IntegerField(source='shipment_packaging_id')
    ...
    def get_crate(self, obj):
        return CrateSerializer(obj.shipment_packaging.crate).data

    def get_status(self, obj):
        return obj.get_package_status_display()

    class Meta:
        model = LastMileTripShipmentPackages
        fields = ('id', 'crate', 'status', 'is_return_verified')
...
class DispatchItemsSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if package_mapping.status != ShipmentPackaging.DISPATCH_STATUS_CHOICES.PACKED:
            raise serializers.ValidationError("Package already marked verified")
        return data
    ...
    def post_package_movement_to_dispatch(self, package_instance):
        ...
        if package_instance.shipment.shipment_status == OrderedProduct.READY_TO_SHIP and \
                not package_instance.shipment.shipment_packaging.filter(
                    status=ShipmentPackaging.DISPATCH_STATUS_CHOICES.PACKED).exists():
        ...
        if 'trip_status' in self.initial_data and self.initial_data['trip_status']:
            ...
            if trip_status == DispatchTrip.VERIFIED:
                if DispatchTripShipmentPackages.objects.filter(
                        ...
                        package_status__in=[DispatchTripShipmentPackages.UNLOADED,
                                            DispatchTripShipmentPackages.PARTIALLY_VERIFIED]).exists():
                    raise serializers.ValidationError(
...
class VerifyShipmentPackageSerializer(serializers.ModelSerializer):
    is_return_verified = serializers.SerializerMethodField()
    shipment_packaging = DispatchItemsSerializer(read_only=True)
...
class VerifyRescheduledShipmentPackageSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if 'trip_id' in self.initial_data and self.initial_data['trip_id']:
            ...
            trip_shipment = LastMileTripShipmentMapping.objects.filter(trip=trip, shipment=shipment).last()
        else:
        ...
        if 'return_status' in self.initial_data and self.initial_data['return_status']:
            if self.initial_data['return_status'] == PACKAGE_VERIFY_CHOICES.OK:
                ...
                raise serializers.ValidationError(
                    "'return_status' | Invalid status for the selected shipment packaging.")
            if shipment_package.package_status == status:
                raise serializers.ValidationError(f"Packaging already marked {status}")

            if status in [LastMileTripShipmentPackages.RETURN_MISSING, LastMileTripShipmentPackages.RETURN_DAMAGED]:
                if 'return_remark' in self.initial_data and self.initial_data['return_remark']:
...
class SummarySerializer(serializers.Serializer):
    ...
    total_empty_crate_check = serializers.IntegerField()
...
class DispatchCenterShipmentPackageSerializer(serializers.ModelSerializer):
    ...
    def get_trip_loading_status(self, obj):
        return obj.trip_packaging_details.last().package_status \
            if obj.trip_packaging_details.filter(
            ~Q(package_status=DispatchTripShipmentPackages.CANCELLED)).exists() else None
...
class LastMileTripShipmentsSerializer(serializers.Serializer):
    ...
    def get_invoices(self, obj):
        return LastMileTripInvoicesBasicDetailsSerializer(obj['invoices'], many=True).data
...
class DeliveryReturnOrderSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('id', 'return_no', 'buyer_shop','seller_shop' ,
                  'return_status', 'return_challan_no','return_order_products')
        depth = 1
        ...
        if mapping_instance.ordered_product.packaged_at != mapping_instance.ordered_product.order.seller_shop_id:
            if 'packaging' in self.initial_data and self.initial_data['packaging']:
                ...
                for package_obj in self.initial_data['packaging']:
                    if 'type' not in package_obj or not package_obj['type']:
                        raise serializers.ValidationError("'package type' | This is mandatory")
                    if package_obj['type'] not in [ShipmentPackaging.CRATE, ShipmentPackaging.SACK,
                                                   ShipmentPackaging.BOX]:
                        raise serializers.ValidationError("'packaging type' | Invalid packaging type")
                    if package_obj['type'] == ShipmentPackaging.CRATE:
                        validate_crates = validate_shipment_crates_list(package_obj, current_shop_id,
                                                                        mapping_instance.ordered_product)
...
class ShipmentCratesValidatedSerializer(serializers.ModelSerializer):
    ...
    def get_all_returned_crates_validated(obj):
        if not obj.last_mile_trip_shipment.last().last_mile_trip_shipment_mapped_packages.filter(
                package_status=LastMileTripShipmentPackages.LOADED,
                shipment_packaging__packaging_type=ShipmentPackaging.CRATE).exists():
            return True
        return False

    class Meta:
        model = OrderedProduct
        fields = ('id', 'status', 'invoice_no', 'all_returned_crates_validated')
...
class ShipmentCompleteVerifySerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = OrderedProduct
        fields = ('id', 'order', 'status', 'invoice_no')
    ...
    def validate(self, data):
        ...
        if 'id' in self.initial_data and self.initial_data['id']:
            ...
            if shipment.last_mile_trip_shipment.last().last_mile_trip_shipment_mapped_packages.filter(
                    package_status=LastMileTripShipmentPackages.LOADED,
                    shipment_packaging__packaging_type=ShipmentPackaging.CRATE).exists():
                raise serializers.ValidationError(f"All crates are not verified for this shipment")
...
class OrderPaymentStatusChangeSerializers(serializers.ModelSerializer):
    ...
    def validate(self, data):
        if 'id' in self.initial_data and self.initial_data['id']:
            if self.context.get('app-type', None) != 4:
                ...
                if RetailerOrderedProductMapping.objects.filter(id=self.initial_data['id'],
                    ...
                    order_instance = RetailerOrderedProductMapping.objects.filter(
                        id=self.initial_data['id']).last().ordered_product.order
                else:
                    raise serializers.ValidationError(f"Order not found for Id {self.initial_data['id']}.")
        ...
        if self.context.get('app-type', None) == 3 and order_instance.order_status == Order.PAYMENT_PENDING and \
            ...
            if self.context.get('sub-app-type',
                                None) == 'ecom' and order_instance.order_status == Order.PAYMENT_PENDING and \
                    order_status not in [Order.PAYMENT_FAILED, Order.PAYMENT_APPROVED, Order.PAYMENT_COD]:
                raise serializers.ValidationError(f"Please Provide valid Payment status")
            elif self.context.get('sub-app-type',
                                  None) == 'pos' and order_instance.order_status == Order.PAYMENT_PENDING and \
                    order_status != Order.PAYMENT_COD:
                raise serializers.ValidationError(f"Please Provide valid Payment status")
...
class PackagesUnderTripSerializer(serializers.ModelSerializer):
    ...
    def get_trip_loading_status(self, obj):
        if obj.last_mile_trip_packaging_details.exists():
            return obj.last_mile_trip_packaging_details.last().package_status
        elif obj.trip_packaging_details.filter(~Q(package_status=DispatchTripShipmentPackages.CANCELLED)).exists():
            return obj.trip_packaging_details.last().package_status
        else:
            return None
                ...
                for batch_detail in return_details:
                    ...
                    if putaway_qty == 0:
                        ...
                        if batch_detail.damaged_qty > 0:
                            ...
                            create_putaway(warehouse, product, batch_id, None, type_damaged, 'RETURNED',
                                           putaway_type_id,
...
class LoadLastMileInvoiceSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        try:
            ...
            if trip_shipment.shipment.shipment_status in [OrderedProduct.MOVED_TO_DISPATCH, OrderedProduct.RESCHEDULED,
                                                          OrderedProduct.NOT_ATTEMPT]:
                trip_shipment.shipment.shipment_status = OrderedProduct.READY_TO_DISPATCH
                trip_shipment.shipment.save()
        except Exception as e:
        ...
        for product_batch in self.initial_data['batch_details']:
            ...
            if not self.instance.ordered_product.rt_ordered_product_mapping.filter(
                ...
                raise serializers.ValidationError("'Invalid Quantity' | Sum of returned quantity and damaged quantity "
                    ...
                    (already_verified_mappings.aggregate(
                        tota_returned_and_damaged=Sum(F('return_qty') + F('damaged_qty'))) \
                             .get('tota_returned_and_damaged', 0) + return_qty + damaged_qty) > (
                    batch_return_qty + batch_damaged_qty):
...
class SuperStoreOrderListSerializer(serializers.ModelSerializer):
    ...
    def get_qty_and_total_amount(self, instance):
        cart_product = instance.ordered_product.order.ordered_cart.rt_cart_list.filter(
            cart_product=instance.product).last()
        if cart_product:
            return {
    ...
    def get_order_status(self, instance):
        retailer_order = instance.ordered_product.order.ref_order.filter(
            ordered_cart__rt_cart_list__cart_product=instance.product).last()
        ...
        if order_status in [Order.PAYMENT_FAILED,
            ...
            if retailer_order_status in [Order.PICKING_ASSIGNED,
                ...
                if shipment.is_returned:
                    return_order = shipment.shipment_return_orders.filter(
                        return_order_products__product=instance.product,
    ...
    def get_return_address(self, instance):
        if instance.buyer_shop:
            return AddressSerializer(
                instance.buyer_shop.shop_name_address_mapping.filter(address_type='shipping').last()).data
        else:
            return None
