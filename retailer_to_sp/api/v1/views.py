    ...
    def post(self, request, format=None):
        ...
        try:
            ...
            try:
                ...
                # if shop mapped with gf
                    ...
                    '''
                    grn = GRNOrderProductMapping.objects.filter(
                        grn_order__order__ordered_cart__gf_shipping_address__shop_name=parent_mapping.parent, available_qty__gt=0, expiry_date__gt=today).values('product_id')
                    cart = GramMappedCart.objects.filter(
                        last_modified_by=self.request.user, cart_status__in=['active', 'pending']).last()
                    if cart:
                        cart_products = cart.rt_cart_list.all()
                        cart_check = True
        ...
        if category:
            product_ids = ProductCategory.objects.filter(
                product__in=grn, category__in=category).values_list('product_id')
            products = products.filter(pk__in=product_ids)
        if keyword:
            products = products.annotate(search=SearchVector(
                'product_name', 'product_brand__brand_name', 'product_long_description')).filter(search=keyword)

        if is_store_active is False:
            products_price = ProductPrice.objects.filter(product__in=products).order_by(
                'product', '-created_at').distinct('product')
        else:
            products_price = ProductPrice.objects.filter(product__in=products, shop=parent_mapping.parent).order_by(
                'product', '-created_at').distinct('product')

        if sort_preference:
            if sort_preference == 'low':
                products_price = products_price.order_by(
                    'price_to_retailer').distinct()
        ...
        if offset and pro_count:
            products_price = products_price[
                int(offset):int(offset) + int(pro_count)]
        ...
        for p in products_price:
            ...
            ptr = round(p.price_to_retailer,
                        2) if p.price_to_retailer else p.price_to_retailer
...
class AddToCart(APIView):
    ...
    def post(self, request):
        ...
        msg = {'is_success': False, 'message': [
            'Sorry no any mapping with any shop!'], 'response_data': None}
        ...
        if Shop.objects.filter(id=shop_id).exists():
            ...
            if parent_mapping.parent.shop_type.shop_type == 'sp':
                if Cart.objects.filter(last_modified_by=self.request.user,
                    ...
                    cart = Cart(last_modified_by=self.request.user,
                                cart_status='active')
                    cart.save()
                ...
                if int(qty) == 0:
                    if CartProductMapping.objects.filter(cart=cart, cart_product=product).exists():
                        CartProductMapping.objects.filter(
                            cart=cart, cart_product=product).delete()

                else:
                    cart_mapping, _ = CartProductMapping.objects.get_or_create(
                        cart=cart, cart_product=product)
                    cart_mapping.qty = qty
                    cart_mapping.save()

                if cart.rt_cart_list.count() <= 0:
                    msg = {'is_success': False, 'message': [
                        'Sorry no any product yet added to this cart'], 'response_data': None}
                else:
                    ...
                    msg = {'is_success': True, 'message': [
                        'Data added to cart'], 'response_data': serializer.data}
                return Response(msg, status=status.HTTP_200_OK)
            ...
            #  if shop mapped with gf
                ...
                if GramMappedCart.objects.filter(last_modified_by=self.request.user,
                    ...
                    cart = GramMappedCart(
                        last_modified_by=self.request.user, cart_status='active')
                    cart.save()

                if int(qty) == 0:
                    if GramMappedCartProductMapping.objects.filter(cart=cart, cart_product=product).exists():
                        GramMappedCartProductMapping.objects.filter(
                            cart=cart, cart_product=product).delete()
                ...
                if cart.rt_cart_list.count() <= 0:
                    ...
                    serializer = GramMappedCartSerializer(GramMappedCart.objects.get(
                        id=cart.id), context={'parent_mapping_id': parent_mapping.parent.id})
...
class CartDetail(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        msg = {
            'is_success': False,
            'message': ['Sorry shop or shop mapping not found'],
            'response_data': None
        }
            ...
            if Cart.objects.filter(
                ...
                if cart.rt_cart_list.count() <= 0:
                    msg = {
                        'is_success': False,
                        'message': [
                            'Sorry no any product yet added to this cart'
                        ],
                        'response_data': None
                    }
                else:
                ...
                msg = {
                    'is_success': False,
                    'message': ['Sorry no any product yet added to this cart'],
                    'response_data': None
                }
                return Response(msg, status=status.HTTP_200_OK)
        ...
        # if shop mapped with gf
            ...
            if GramMappedCart.objects.filter(
                last_modified_by=self.request.user,
                cart_status__in=['active', 'pending']
            ).exists():
                cart = GramMappedCart.objects.filter(
            ...
            msg = {
                'is_success': False,
                'message': [
                    'Sorry shop is not associated with any Gramfactory or any SP'
                ],
                'response_data': None
            }
            return Response(msg, status=status.HTTP_200_OK)
    ...
    def post(self, request):
        ...
        if parent_mapping.parent.shop_type.shop_type == 'sp':
            if Cart.objects.filter(last_modified_by=self.request.user, cart_status__in=['active', 'pending']).exists():
                ...
                for cart_product in cart_products:
                    ...
                    is_error = False
                    ordered_amount = int(
                        cart_product.qty) * int(cart_product.cart_product.product_inner_case_size)
                    ...
                    if available_qty and int(available_qty) >= ordered_amount:
                        ...
                        serializer = CartSerializer(
                            cart, context={'parent_mapping_id': parent_mapping.parent.id})
                        msg = {'is_success': True, 'message': [
                            ''], 'response_data': serializer.data}
                    else:
                        msg = {'is_success': False, 'message': [
                            'available_qty is none'], 'response_data': None}
                        if int(available_qty) < ordered_amount:
                            cart_product.qty_error_msg = ERROR_MESSAGES[
                                'AVAILABLE_PRODUCT'].format(int(available_qty))
                            cart_product.save()
                            serializer = CartSerializer(
                                cart, context={'parent_mapping_id': parent_mapping.parent.id})
                            msg = {'is_success': True, 'message': [
        ...
        # if shop mapped with gf
            ...
            if GramMappedCart.objects.filter(last_modified_by=self.request.user,
                ...
                cart = GramMappedCart.objects.filter(last_modified_by=self.request.user,
                                                     cart_status__in=['active', 'pending']).last()
                cart_products = GramMappedCartProductMapping.objects.filter(
                    cart=cart)
                ...
                for cart_product in cart_products:
                    ...
                    ordered_product_sum = ordered_product_details.aggregate(
                    ...
                    if ordered_product_sum['available_qty_sum'] is not None:
                        if int(ordered_product_sum['available_qty_sum']) < int(cart_product.qty) * int(cart_product.cart_product.product_inner_case_size):
                            available_qty = int(ordered_product_sum[
                                                'available_qty_sum'])
                            cart_product.qty_error_msg = ERROR_MESSAGES[
                                'AVAILABLE_PRODUCT'].format(int(available_qty))
                            is_error = True
...
class CreateOrder(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        shop_id = self.request.POST.get('shop_id')
        msg = {'is_success': False, 'message': [
            'Have some error in shop or mapping'], 'response_data': None}
        ...
        if parent_mapping.parent.shop_type.shop_type == 'sp':
            ...
            if Cart.objects.filter(last_modified_by=self.request.user, id=cart_id).exists():
                cart = Cart.objects.get(
                    last_modified_by=self.request.user, id=cart_id)
                cart.cart_status = 'ordered'
                cart.save()

                if OrderedProductReserved.objects.filter(cart=cart).exists():
                    order, _ = Order.objects.get_or_create(
                        last_modified_by=request.user, ordered_by=request.user, ordered_cart=cart, order_no=cart.order_id)
                    ...
                    serializer = OrderSerializer(order, context={
                                                 'parent_mapping_id': parent_mapping.parent.id, 'current_url': current_url})
                    msg = {'is_success': True, 'message': [
                        ''], 'response_data': serializer.data}
                else:
                    msg = {'is_success': False, 'message': [
        ...
        # if shop mapped with gf
            ...
            if GramMappedCart.objects.filter(last_modified_by=self.request.user, id=cart_id).exists():
                cart = GramMappedCart.objects.get(
                    last_modified_by=self.request.user, id=cart_id)
                cart.cart_status = 'ordered'
                cart.save()

                if GramOrderedProductReserved.objects.filter(cart=cart).exists():
                    order, _ = GramMappedOrder.objects.get_or_create(
                        last_modified_by=request.user, ordered_by=request.user, ordered_cart=cart, order_no=cart.order_id)
                    ...
                    serializer = GramMappedOrderSerializer(order, context={
                                                           'parent_mapping_id': parent_mapping.parent.id, 'current_url': current_url})
                    msg = {'is_success': True, 'message': [
                        ''], 'response_data': serializer.data}
                else:
...
class OrderList(generics.ListAPIView):
    ...
    def list(self, request):
        ...
        shop_id = self.request.GET.get('shop_id')
        msg = {'is_success': False, 'message': [
            'Data Not Found'], 'response_data': None}
        ...
        if parent_mapping.parent.shop_type.shop_type == 'sp':
            queryset = Order.objects.filter(
                last_modified_by=user).order_by('-created_at')

            serializer = OrderSerializer(queryset, many=True, context={
                                         'parent_mapping_id': parent_mapping.parent.id, 'current_url': current_url})
        elif parent_mapping.parent.shop_type.shop_type == 'gf':
            queryset = GramMappedOrder.objects.filter(
                last_modified_by=user).order_by('-created_at')
            serializer = GramMappedOrderSerializer(queryset, many=True, context={
                                                   'parent_mapping_id': parent_mapping.parent.id, 'current_url': current_url})
...
class OrderDetail(generics.RetrieveAPIView):
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        if parent_mapping.parent.shop_type.shop_type == 'sp':
            ...
            serializer = OrderSerializer(queryset, context={
                                         'parent_mapping_id': parent_mapping.parent.id, 'current_url': current_url})
        elif parent_mapping.parent.shop_type.shop_type == 'gf':
            queryset = GramMappedOrder.objects.get(id=pk)
            serializer = GramMappedOrderSerializer(queryset, context={
                                                   'parent_mapping_id': parent_mapping.parent.id, 'current_url': current_url})
...
class DownloadInvoiceSP(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        shop = a
        products = a.rt_order_product_order_product_mapping.filter(
            shipped_qty__gt=0)
        ...
        for m in products:
            ...
            if all_tax_list.exists():
                ...
                basic_rate = (float(product_pro_price_ptr)) / \
                    (float(get_tax_val) + 1)
                base_price = (float(product_pro_price_ptr) *
                              float(m.shipped_qty)) / (float(get_tax_val) + 1)
...
class DownloadDebitNote(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        data = {"object": order_obj,
                "order": order_obj.order, "products": products}
...
class CustomerCareApi(APIView):
    ...
    def get(self, request):
        ...
        msg = {'is_success': True, 'message': [
            'All Messages'], 'response_data': serializer.data}
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            serializer.save()
            msg = {'is_success': True, 'message': [
                'Message Sent'], 'response_data': serializer.data}
            return Response(msg, status=status.HTTP_201_CREATED)
...
class CustomerOrdersList(APIView):
    ...
    def get(self, request):
        ...
        msg = {'is_success': True, 'message': [
            'All Orders of the logged in user'], 'response_data': serializer.data}
        return Response(msg, status=status.HTTP_201_CREATED)
        # else:
        # return Response(msg, status=status.HTTP_201_CREATED)
...
class PaymentApi(APIView):
    ...
    def get(self, request):
        ...
        msg = {'is_success': True, 'message': [
            'All Payments'], 'response_data': serializer.data}
        return Response(msg, status=status.HTTP_201_CREATED)
        ...
        if parent_mapping.parent.shop_type.shop_type == 'sp':
            if OrderedProductReserved.objects.filter(cart__id=cart_id, reserve_status='reserved').exists():
                for ordered_reserve in OrderedProductReserved.objects.filter(cart__id=cart_id, reserve_status='reserved'):
                    ...
                    ordered_reserve.delete()
            msg = {'is_success': True, 'message': [
                'Blocking has released'], 'response_data': None}
        elif parent_mapping.parent.shop_type.shop_type == 'gf':
            if GramOrderedProductReserved.objects.filter(cart__id=cart_id, reserve_status='reserved').exists():
                for ordered_reserve in GramOrderedProductReserved.objects.filter(cart__id=cart_id, reserve_status='reserved'):
                    ordered_reserve.order_product_reserved.available_qty = int(
