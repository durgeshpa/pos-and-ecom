...
class CartCentral(GenericAPIView):
    ...
    def post_basic_validate(self, shop, cart_id=None):
        ...
        if not self.request.data.get('product_id'):
            ...
            # Provided info check
            name, sp, ean = self.request.data.get('product_name'), self.request.data.get(
                'selling_price'), self.request.data.get('product_ean_code')
            ...
            try:
                ...
                return {'error': "Product Not Found!"}
        return {'product': product, 'new_product_info': new_product_info, 'quantity': qty, 'cart': cart}
