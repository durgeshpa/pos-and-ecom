...
tcs_config_params = get_config('TCS_CONFIG', None)
...
def populate_buyer_purchase_data():
    ...
    while start_year <= 2022:
        ...
        total_purchase = Invoice.objects.filter(~Q(shipment__order__buyer_shop__id__in=[33887, 50507, 50482, 1394, 599]),
                                                shipment__order__seller_shop__id__in=[50484, 600, 1393],
        ...
        for data in total_purchase:
            BuyerPurchaseData.objects.create(seller_shop_id=data['shipment__order__seller_shop_id'],
                                             ...
                                             fin_year=start_year, total_purchase=round(data['total_purchase'], 2),
                                             )
        start_year += 1
...
def populate_tcs_amount():
    ...
    for data in buyer_data:
        is_buyer_pan_available = ShopDocument.objects.filter(shop_name_id=data.buyer_shop_id,
                                                          shop_document_type=ShopDocument.PAN).exists()
        tcs_percent = 1
        if is_buyer_pan_available:
            tcs_percent = 0.1
        ...
        invoices = Invoice.objects.filter(shipment__order__seller_shop_id=data.seller_shop_id,
                                          shipment__order__buyer_shop_id=data.buyer_shop_id,
                                          created_at__gte=fin_year_start_date,
        ...
        for i in invoices:
            if total_purchase >= tcs_applicable_amt:
                ...
                tcs_amount = round(i.invoice_sub_total*tcs_percent/100, 2)
                print(tcs_amount)
                i.tcs_amount = tcs_amount
                i.invoice_total = i.invoice_sub_total + tcs_amount
                i.save()
...
def populate_credit_note_tcs():
    ...
    for note in notes:
        ...
        note.tcs_amount = round(note.amount * tcs_percent, 2)
        note.note_total = note.amount * note.tcs_amount
        note.save()