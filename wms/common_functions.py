...
class CommonWarehouseInventoryFunctions(object):
    ...
    def create_warehouse_inventory(cls, warehouse, sku, inventory_type, inventory_state, quantity, in_stock, weight=0):
        ...
        if ware_house_inventory_obj:
            ...
            info_logger.info(f"warehouse.id {warehouse.id} ,sku.product_sku {sku.product_sku} ,"
                             f"inventory_type.inventory_type "
                             f"{inventory_type.inventory_type} ,inventory_state.inventory_state "
                             f"{inventory_state.inventory_state} " f",ware_house_quantity {ware_house_quantity} ,"
                             f"ware_house_inventory_obj {ware_house_inventory_obj} ")
...
def assign_clickable_state(user):
    ...
    """
    instance = PickerDashboard.objects.filter(picker_boy_id=user, picking_status='picking_assigned').order_by('created_at').first()
