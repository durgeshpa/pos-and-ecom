...
def get_warehouse_stock(shop_id=None, product=None, inventory_type=None):
    ...
    if shop_id:
        ...
        if product is None:
            product_dict_grocery = get_stock(shop, inventory_type)
            product_dict_superstore = get_all_superstore_products(shop)
            product_dict = {**product_dict_grocery, **product_dict_superstore}
        else:
    ...
    info_logger.info(
        "inside get_warehouse_stock, products: " + str(products) + ", product_price_dict: " + str(product_price_dict))
    for product in products:
        ...
        visible = False
        if product_dict:
            warehouse_visible = WarehouseInventory.objects.filter(warehouse=shop, sku=product,
                                                                  inventory_state=InventoryState.objects.filter(
                                                                      inventory_state='total_available').last(),
                                                                  inventory_type=type_normal).last()
