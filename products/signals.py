...
def update_parent_category_elasticsearch(sender, instance=None, created=False, **kwargs):
    update_product_on_category_update(instance)
...
def update_product_on_category_update(instance):
    ...
    for category in parent_pro_categories:
        ...
        for product in child_products:
            for prod_price in product.product_pro_price.filter(status=True, seller_shop__id__in=['600', '172']
                                                               ).distinct('seller_shop').values('seller_shop', 'product'):
...
def update_parent_brand_elasticsearch(sender, instance=None, created=False, **kwargs):
    update_product_on_brand_update(instance)
...
def update_product_on_brand_update(instance):
