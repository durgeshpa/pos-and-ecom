...
def update_product_elasticsearch(sender, instance=None, created=False, **kwargs):
    if instance.status==False:
        for prod_price in instance.product_pro_price.filter(status=True).values('seller_shop', 'product'):
            update_shop_product_es(prod_price['seller_shop'], prod_price['product'], name=instance.product_name)
            update_shop_product_es(prod_price['seller_shop'], prod_price['product'], pack_size=instance.product_inner_case_size)
            update_shop_product_es(prod_price['seller_shop'], prod_price['product'], product_status=instance.status)
