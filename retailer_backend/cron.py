...
class CronToDeleteOrderedProductReserved(APIView):
    ...
    def get(self, request):
        queryset = OrderedProductReserved.objects.filter(order_reserve_end_time__lte=timezone.now(),reserve_status='reserved')
        queryset.update(
            order_product_reserved__available_qty=F('order_product_reserved__available_qty')+F('ordered_reserve.reserved_qty'),
            cart__cart_status='pending',
            reserve_status = 'free'
            )
        return Response()
