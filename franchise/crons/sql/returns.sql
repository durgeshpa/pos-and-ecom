Select Distinct SalesReturn.Date as SR_Date, SalesReturn.Number as SR_Number,
    SalesReturn_SalesReturnItem_Item.Name, -(SalesReturn_SalesReturnItem.Quantity) as ItemtotalQty,
    (SalesReturn_SalesReturnItem.TotalAmount) as TotalAmount,
    SalesReturn_SalesReturnItem_Item_Category.Name as CategoryName, SalesReturn_SalesReturnItem_Item.Barcode, SalesReturn_BusinessLocation.Id as bl_id,SalesReturn_BusinessLocation.LocationName as BS_Location_Name, SalesReturn_SalesInvoice.Date, SalesReturn_SalesInvoice.InvNumber, SalesReturn_Customer.Name as CustomerName, SalesReturn_Customer_Address_Contact.MobileNumber As MobileNumber
    ...
    --Inner join tbl_DYN_BusinessLocations BusinessLocation on SalesReturn_BusinessLocation.Id = BusinessLocation.Id
    where convert(date,SalesReturn.date) >=