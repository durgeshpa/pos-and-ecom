...
class CatalogueProductCreation(GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            shop = Shop.objects.filter(id=request.data.get('shop_id'))
            id = shop.values()[0].get('shop_owner_id')
    ...
    def put(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            retailer_product = RetailerProduct.objects.filter(id=request.data.get('product_id'))
            shop = Shop.objects.filter(id=retailer_product.values()[0].get('shop_id'))
            id = shop.values()[0].get('shop_owner_id')
            if request.user.id == id:
