        ...
        for row in uploaded_data_by_user_list:
            ...
            if row.get('product_id') == '':
                ...
                PosInventoryCls.stock_inventory(r_product.id, PosInventoryState.NEW, PosInventoryState.AVAILABLE,
                                                round(Decimal(row.get('quantity')), 3), uploaded_by,
                                                r_product.sku,
...
def upload_retailer_products_list(request):
    ...
    if request.method == 'POST':
        form = RetailerProductsCSVUploadForm(request.POST, request.FILES, shop_id=shop_id, user=request.user)
...
def DownloadRetailerCatalogue(request, *args, **kwargs):
    ...
    writer.writerow(
        ['product_id', 'shop_id', 'shop_name', 'product_sku', 'product_name', 'mrp', 'selling_price',
         'linked_product_sku', 'product_ean_code', 'description', 'sku_type','parent_product_id' , 'category',
         'sub_category', 'brand', 'sub_brand', 'status', 'quantity', 'discounted_sku', 'discounted_stock',
         'discounted_price', 'product_pack_type', 'measurement_category', 'purchase_pack_size',
         'available_for_online_orders', 'online_order_price', 'is_visible', 'offer_price', 'offer_start_date',
         'offer_end_date', 'initial_purchase_value'])
    ...
    if product_qs.exists():
        ...
        for product_id, product in product_dict.items():
            ...
            category = None
            sub_category = None
            product_type = product['linked_product__parent_product__product_type']
            ...
            if product['linked_product__product_sku'] and (product_type =='b2c' or product_type == 'both'):
                category = b2c_category
                sub_category = b2c_sub_category
            writer.writerow(
    ...
    def total_order_calculation(self, user, start_date, end_date, shop):
        ...
        ecom_cash_order_qs = OrderedProduct.objects.filter(invoice__created_at__date__gte=start_date,
                                                           ...
                                                           order__order_status__in=[
                                                               ...
                                                               RetailerOrderedReport.PARTIALLY_RETURNED,
                                                               RetailerOrderedReport.FULLY_RETURNED])\
            .exclude(order__order_status=RetailerOrderedReport.CANCELLED)

        ecom_cash_order_amt = 0
        for e_cash_invoice in ecom_cash_order_qs:
            ecom_cash_order_amt += e_cash_invoice.invoice_amount_final
        ...
        ecom_online_order_amt = 0
        for e_online_invoice in ecom_online_order_qs:
            ecom_online_order_amt += e_online_invoice.invoice_amount_final
        ...
        pos_cash_amt = Decimal(pos_cash_order_amt) - Decimal(pos_cash_return_amt)
        pos_online_amt = Decimal(pos_online_order_amt) - Decimal(pos_online_return_amt)
        ecomm_online_amt = Decimal(ecom_online_order_amt) - Decimal(ecom_online_return_amt)
        ecomm_cash_amt = Decimal(ecom_cash_order_amt) - Decimal(ecom_cash_return_amt)
...
class RetailerPurchaseReportView(View):
    ...
    def get(self, *args, **kwargs):
        ...
        if error:
            return render(
                self.request,
                'admin/services/product-purchase-value-report.html',
                {'form': RetailerPurchaseReportForm(initial=self.request.GET)}
            )

        products = RetailerProduct.objects.filter(shop=shop).values('id', 'shop', 'name', 'sku',
                                                                    'linked_product__product_sku',
                                                                    'product_ean_code', 'purchase_pack_size',
                                                                    'initial_purchase_value')
        response = HttpResponse(content_type='text/csv')
        response['Content-Disposition'] = 'attachment; filename="purchase-value-report.csv"'
        writer = csv.writer(response)
        shop_obj = Shop.objects.filter(id=shop, shop_type__shop_type='f', status=True, approval_status=2,
                                       pos_enabled=True, pos_shop__status=True).last()
        writer.writerow(['Shop Name:', shop_obj.__str__()])
        writer.writerow([])
        writer.writerow(['Sno', 'Product Name', 'Retailer SKU', 'Linked Product SKU', 'EAN',
                         'Purchase Pack Size', 'Quantity', 'Purchase Value', 'Total Purchase Value'])

        for i, product in enumerate(products):
            pos_inv_products_map = PosInventory.objects.filter(
                product__id=product['id'], inventory_state=PosInventoryState.objects.filter(
                    inventory_state=PosInventoryState.AVAILABLE).last()).last()
            ...
            writer.writerow(
                [(i + 1), product['name'], product['sku'], product['linked_product__product_sku'],
                 product['product_ean_code'], product['purchase_pack_size'], quantity,
                 product['initial_purchase_value'], total_purchase_value])
        return response


class RetailerPurchaseReportFormView(View):
    def get(self, request):
        form = RetailerPurchaseReportForm()
        return render(
            self.request,
            'admin/services/product-purchase-value-report.html',
            {'form': form}
...
def products_list_status(request, product_status_info):
    p = re.compile('(?<!\\\\)\'')
    ...
    bulk_product_obj = BulkRetailerProduct.objects.filter(bulk_no=product_status_info['bulk_no'])
    if bulk_product_obj:
        csv_file_name = bulk_product_obj.values()[0]['products_csv']
    else:
        info_logger.info(f"[pos/views.py:products_list_status] - [bulk_no : {product_status_info['bulk_no']}]")

    try:
        s3 = boto3.resource('s3', aws_access_key_id=config('AWS_ACCESS_KEY_ID'),
                            aws_secret_access_key=config('AWS_SECRET_ACCESS_KEY'))
        info_logger.info(f"[pos/views.py:products_list_status] - Successfully connected with s3")
    except ClientError as err:
        error_logger.error(f"[pos/views.py:products_list_status] Failed to connect with s3 - {err}")
        raise err
    ...
    try:
        res = obj.get()
        info_logger.info(f"[products/views.py:products_list_status] Successfully get the response from s3")
    except ClientError as err:
        error_logger.error(f"[products/views.py:products_list_status] Failed to get the response from s3 - {err}")
        raise err
