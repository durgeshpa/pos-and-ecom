...
def bulk_product_validation_with_error_list_and_validated_data(products_csv, shop_id):
    RetailerProduct = apps.get_model('pos.RetailerProduct')
    MeasurementCategory = apps.get_model('pos.MeasurementCategory')
    MeasurementUnit = apps.get_model('pos.MeasurementUnit')
    ...
    reader = csv.reader(codecs.iterdecode(products_csv, 'utf-8', errors='ignore'))
    headers = next(reader)
    error_list.append(list(headers) + ["upload_status"])
    data_list = read_file_to_convert_into_list(headers, reader)
    for row_num, row in enumerate(data_list):
        row_num += 2
        error_msg = []
        ...
        if 'offer_price' in row.keys() and row['offer_price'] and \
            ...
            try:
                datetime.strptime(row['offer_start_date'], '%Y-%m-%d').date()
            except:
                error_msg.append("Invalid format of Offer Start Date")

            if not 'offer_end_date' in row.keys() or not row['offer_end_date']:
                error_msg.append("Offer End Date is missing")
            try:
                datetime.strptime(row['offer_end_date'], '%Y-%m-%d').date()
            except:
                error_msg.append("Invalid format of Offer End Date")
