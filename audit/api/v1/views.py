...
from wms.models import BinInventory, Bin, InventoryType, PickupBinInventory
...
class AuditListView(APIView):
    ...
    def get(self, request):
        ...
        # if request.user.has_perm('audit.change_audit')
...
def update_inventory(batch_id, bin_id, physical_inventory):
...
class AuditInventory(APIView):
    ...
    def get(self, request):
        ...
        if not batch_id:
            ...
            return Response(msg, status=status.HTTP_200_OK)
        bin_inventory_dict = self.get_bin_inventory(batch_id, bin)
        data = {'bin_id': bin_id, 'batch_id': batch_id,
                'inventory': bin_inventory_dict}
        msg = {'is_success': True, 'message': 'OK', 'data': data}
        return Response(msg, status=status.HTTP_200_OK)

    def get_bin_inventory(self, batch_id, bin):
        ...
        PickupBinInventory.objects.filter(bin=bin, batch_id=batch_id,)
    ...
    def post(self, request):
        ...
        current_inventory = self.get_bin_inventory(self, batch_id, bin)
        ...
        for key, value in current_inventory.items():
            ...
            is_update_done = update_inventory(batch_id, bin_id, physical_inventory)
