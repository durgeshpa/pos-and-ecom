...
from wms.models import WarehouseInventory, WarehouseInternalInventoryChange, InventoryType, InventoryState, BinInventory, BinInternalInventoryChange
...
def index(request):
    start_automated_inventory_audit()
...
def run_warehouse_level_audit(audit_run):
    ...
    last_day_inventory = WarehouseInventoryHistoric.objects.filter(warehouse=audit_run.warehouse,
                                                                   ...
                                                                   archived_at__lte=audit_started).values('sku_id', 'inventory_type_id', 'inventory_state_id', 'quantity')
    info_logger.debug("Audit run {} : Fetched last days snapshot, total rows {}" .format(audit_run, last_day_inventory.count()))
    ...
    info_logger.debug("Audit run {} : Fetched last days transactions, total rows {}".format(audit_run, last_day_transactions.count()))
...
def run_bin_level_audit(audit_run):
    ...
    current_inventory = BinInventory.objects.filter(warehouse=audit_run.warehouse).values('sku_id', 'batch_id', 'bin_id', 'inventory_type_id', 'quantity')
    last_day_inventory = BinInventoryHistoric.objects.filter(warehouse=audit_run.warehouse,
                                                             archived_at__gte=prev_day,
                                                             archived_at__lte=audit_started).values('sku_id', 'batch_id', 'bin_id', 'inventory_type_id', 'quantity')
    for item in last_day_inventory:
        inventory_calculated[item['sku_id']][item['batch_id']][item['bin_id']][item['inventory_type_id']] = item['quantity']
...
def run_bin_warehouse_integrated_audit(audit_run):
    ...
    for item in current_warehouse_inventory:
        bin_quantity = BinInventory.available_qty_with_inventory_type(audit_run.warehouse.id,
                                                                      item['sku_id'], item['inventory_type_id'])
...
def run_audit(audit_run, inventory_choice):
    if inventory_choice == AUDIT_INVENTORY_CHOICES.WAREHOUSE:
        return run_warehouse_level_audit(audit_run)
    if inventory_choice == AUDIT_INVENTORY_CHOICES.BIN:
        return run_bin_level_audit(audit_run)
    if inventory_choice == AUDIT_INVENTORY_CHOICES.INTEGRATED:
        return run_bin_warehouse_integrated_audit(audit_run)
    return audit_run


def start_automated_inventory_audit():
    audits_to_perform = AuditDetails.objects.filter(audit_inventory_type=AUDIT_RUN_TYPE_CHOICES.AUTOMATED,
    ...
    for audit in audits_to_perform:
        ...
        if audit_run:
            ...
            continue
        audit_run = AuditRun.objects.create(warehouse=audit.warehouse, audit=audit, status=AUDIT_RUN_STATUS_CHOICES.IN_PROGRESS)
        info_logger.debug("Audit run started AuditDetails {} at {} ".format(audit, audit_run.started_at))
        try:
            audit_run = run_audit(audit_run, audit.audit_inventory_type)
        except Exception as e:
            info_logger.error("Audit run aborted with Exception ")
            audit_run.status = "aborted"
        else:
