...
from .serializers import (RetailerTypeSerializer, ShopBasicSerializer, ShopTypeSerializer,
        ...
        PosShopUserMappingCreateSerializer, PosShopUserMappingUpdateSerializer
)
from shops.models import (PosShopUserMapping, RetailerType, ShopType, Shop, ShopPhoto, ShopDocument, ShopUserMapping, SalesAppVersion, ShopRequestBrand, ShopTiming,
    FavouriteProduct, BeatPlanning, DayBeatPlanning, ExecutiveFeedback, USER_TYPE_CHOICES)
...
from shops.common_validators import validate_data_format, validate_id, validate_mapping, validate_psu_id, validate_psu_put
...
class TeamListView(generics.ListAPIView):
    ...
    def list(self, request, *args, **kwargs):
        ...
        if not employee_map.exists():
            ...
            return Response(msg, status=status.HTTP_200_OK)
        employee_map = ShopUserMapping.objects.filter(manager__in=self.get_manager(), status=True, shop__shop_type__shop_type__in=['r', 'f'])
        employee_shop_map = {}
        for e in employee_map:
            employee_shop_map[e.employee] = employee_shop_map[e.employee] + [e.shop.id] if e.employee in employee_shop_map else [e.shop.id]
        ...
        for emp in employee_shop_map:
            store_added = emp.shop_created_by.filter(created_at__date__lte=to_date, created_at__date__gte=from_date).count()
