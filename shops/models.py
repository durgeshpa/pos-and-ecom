...
class ShopUserMapping(models.Model):
    ...
    def save(self, *args, **kwargs):
        if self.manager == self:
            ...
            self.status = True
        if self.status == True and self.employee_group == 'Sales Executive':
            ShopUserMapping.objects.filter(shop=self.shop, employee_group = 'Sales Executive', status=True).update(status=False)
            self.status = True
        super().save(*args, **kwargs)
